...
class CosmoSacGcgcn(CosmoSac):
    def __init__(self, sig, nhb, oh, ot, vol, version=2013):
    ...
    def add_comp(self, smiles, name=None):
        nfm, efm = gcgcn_input(smiles)
        nfm = np.reshape(nfm, [1, *np.shape(nfm)])
        efm = np.reshape(efm, [1, *np.shape(efm)])

        # area = np.sum(self.sig.predict(mat_input))
        volume = 562 * self.vol.predict([nfm, efm], verbose=0)[0][0]
        sigma_profiles = np.zeros((3, 51))
        sigma_profiles[0] = 145 * self.nhb.predict([nfm, efm], verbose=0)[0]
        sigma_profiles[1] = 7 * self.oh.predict([nfm, efm], verbose=0)[0]
        sigma_profiles[2] = 16 * self.ot.predict([nfm, efm], verbose=0)[0]
    ...
    def get_totsig(self, smiles):
        ...
        psigA = 145 * self.sig.predict([nfm, efm], verbose=0)[0]
...
def load_models():
    sig = models.load_model(opj(DIR_PATH, "models", "gcgcn_sig.h5"), compile=False)
    nhb = models.load_model(opj(DIR_PATH, "models", "gcgcn_nhb.h5"), compile=False)
    oh = models.load_model(opj(DIR_PATH, "models", "gcgcn_oh.h5"), compile=False)
    ot = models.load_model(opj(DIR_PATH, "models", "gcgcn_ot.h5"), compile=False)
    vol = models.load_model(opj(DIR_PATH, "models", "gcgcn_vol.h5"), compile=False)

    return sig, nhb, oh, ot, vol


def cal_gamma(SMILES1, SMILES2, x1, x2, T, models=None):
    try:
        T = float(T)
        x1 = float(x1)
        x2 = float(x2)
    ...
    if models is None:
        sig, nhb, oh, ot, vol = load_models()
    else:
        sig, nhb, oh, ot, vol = models

    # Run Scripts
    gcgcn_model = CosmoSacGcgcn(sig, nhb, oh, ot, vol)
    gcgcn_model.add_comp(SMILES1)
    gcgcn_model.add_comp(SMILES2)
    gcgcn_model.x = [x1, x2]
    gcgcn_model.T = 373.15  # K
    a = gcgcn_model.gam()
