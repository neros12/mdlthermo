...
for i in range(10):
    exec(
        f"""
HFORM_{i} = models.load_model(opj("models","HFORM","Model {i} Max=1400.h5"), compile=False)
HFUS_{i} = models.load_model(opj("models","HFUS","Model {i} Max=100.h5"), compile=False)
PC_{i} = models.load_model(opj("models","PC","Model {i} Max=13000.h5"), compile=False)
TBN_{i} = models.load_model(opj("models","TBN","Model {i} Max=1620.h5"), compile=False)
TC_{i} = models.load_model(opj("models","TC","Model {i} Max=2000.h5"), compile=False)
TF_{i} = models.load_model(opj("models","TF","Model {i} Max=550.h5"), compile=False)
TMN_{i} = models.load_model(opj("models","TMN","Model {i} Max=720.h5"), compile=False)
VC_{i} = models.load_model(opj("models","VC","Model {i} Max=1.2.h5"), compile=False)
        """
    )
