# -*- coding: utf-8 -*-
"""
Created on Thu Nov  7 13:54:42 2024.
...
class PR:
    ...
    def add_comp(self, smiles, Tc=None, pc=None, omega=None, c=None):
        ...
        with open('pr_param.json', mode='r') as f:
    ...
    def mix_mhv2(self, smiles, T, x, a, b):
        ...
        q1 = -0.4347
        q2 = -0.003654
        term = -q1 * sum_alpha - q2 * sum_alpha2 -\
            gE - np.sum(x * np.log(b_mix / b))
    ...
    def cal_crit_mix(self, x):
        ...
        for _ in range(50):
            ...
            acmix, bmix = self.mix_mhv2(
                self.smiles, Tcmix, x, self.ac, self.b
            )
    ...
    def set_state(self, T, x):
        ...
        alpha_mc = self.alpha_mc(
            T, self.Tc, self.c[:, 0], self.c[:, 1], self.c[:, 2]
        )
        ...
        amix, bmix = self.mix_mhv2(
            self.smiles, T, x, a, self.b
        )
    ...
    def p(self, rho):
        ...
        return rho*R*T/(1 - b*rho) - a*rho**2/(1 + 2*b*rho - b**2*rho**2)
    ...
    def rho(self, p):
        ...
        solve_liq_omega = spo.fsolve(
            self.obj_dp, rho_hi - 0.1/b, full_output=True
        )
        ...
        # Calculate boundary density
        solve_vap_omega = spo.fsolve(
            self.obj_dp, rho_lo + 0.1/b, full_output=True
        )
    ...
    def phi_hat(self, p):
        ...
        ln_phihat_vap = (
            (B_bar/B)*(Z_vap - 1) - np.log(Z_vap - B) - alpha_bar *
            np.log((Z_vap + (1 + 2**0.5)*B)/(Z_vap + (1 - 2**0.5)*B))/2**1.5
        )
        ln_phihat_liq = (
            (B_bar/B)*(Z_liq - 1) - np.log(Z_liq - B) - alpha_bar *
            np.log((Z_liq + (1 + 2**0.5)*B)/(Z_liq + (1 - 2**0.5)*B))/2**1.5
        )
    ...
    def cal_vle(self, p=None, T=None, x=None, y=None, tol=1e-4, max_iter=500,
                psat=None, z=None):
        ...
        if p is not None and x is not None and T is None and y is None:
            ...
            return self.cal_vle_pT(
                p, T, tol=tol, max_iter=max_iter, psat=psat, z=z
            )
        else:
            raise ValueError(
    ...
    def cal_vle_pT(self, p, T, tol=1e-4, max_iter=500, psat=None, z=None):
        ...
        for iteration in range(max_iter):
            ...
            # Calculate K
            pr.set_state(T, x)
            _, phi_liq = pr.phi_hat(p)

            pr.set_state(T, y)
            phi_vap, _ = pr.phi_hat(p)
            ...
            # print(f"Iteration {iteration}: V = {V:.4f}, ",
            #       f"x = {x}, y = {y}, K = {K}")
